...
type BubbleUp<A extends Record<string, any>> = UnionToIntersection<
  ...
  | {
      [K in NullableChildren<A>]-?: Record<`${Extract<K, string>}?`, NonNullable<A[K]>>;
    }[NullableChildren<A>]
  | {
      [K in ArrayChildren<A>]: Record<
        K extends "" ? "[]>" | "[number]" : `${Extract<K, string>}.${"[]>" | "[number]"}`,
        A[K][number]
      >;
    }[ArrayChildren<A>]
  | {
      [K in NonStructRecordChildren<A>]: Record<
        K extends "" ? "{}>" | "[string]" : `${Extract<K, string>}.${"{}>" | "[string]"}`,
        A[K][number]
      >;
